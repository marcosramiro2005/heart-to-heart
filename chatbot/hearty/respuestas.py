# respuestas.py - Respuestas del chatbot Hearty
# Este archivo contiene todas las respuestas predefinidas del chatbot,
# organizadas por categorías para diferentes emociones y situaciones.

FRASES_MOTIVACIONALES = [ # Frases motivacionales para incluir en respuestas
    "El progreso, no la perfección, es lo que importa. 🌱",
    "Cuidarte no es egoísta, es necesario. 💚",
    "Un día a la vez. Solo el siguiente paso. 👣",
    "Mereces el mismo cuidado que das a los demás. 🌸",
    "Las emociones difíciles no duran para siempre. 🌊",
    "Tu bienestar emocional importa de verdad. 💙",
    "Eres más fuerte de lo que crees ahora mismo. 💪",
    "Cada pequeño paso cuenta. No te rindas. 🌟",
    "Está bien no estar bien. Lo que importa es seguir. 🤍",
    "Hoy sobreviviste todo lo que creías que no podías. ✨",
    "Respirar profundo también es valiente. 🫁",
    "No tienes que tenerlo todo resuelto hoy. 💛",
]

CONSEJOS_GENERALES = [ # Consejos generales de bienestar para respuestas
    "Dedica 5 minutos al día a respirar conscientemente. Marca la diferencia.",
    "Escribe tres cosas positivas antes de dormir. Entrena tu cerebro hacia el bienestar.",
    "El ejercicio moderado es uno de los mejores antidepresivos naturales.",
    "Limita el tiempo en redes sociales. Tu mente lo agradecerá.",
    "Hablar con alguien de confianza alivia más que guardar todo dentro.",
    "La hidratación afecta directamente al estado de ánimo. Bebe agua.",
    "Una rutina de sueño consistente mejora la salud mental enormemente.",
    "La naturaleza tiene un efecto calmante demostrado. Sal a caminar.",
    "Aprende a decir no. Poner límites es un acto de amor propio.",
    "El autocuidado no es un lujo, es una necesidad básica.",
]

RESPUESTAS_ANSIEDAD = [ # Respuestas específicas para cuando se detecta ansiedad
    "Entiendo que la ansiedad puede sentirse abrumadora 💙 Es una respuesta de tu cuerpo que intenta protegerte, aunque en este momento no lo parezca.\n\nLo primero: respira conmigo. Inhala 4 segundos, mantén 7, exhala 8. Tres ciclos y notarás la diferencia.\n\n¿Puedes contarme qué está generando esa ansiedad ahora mismo?",
    "La ansiedad tiene una voz muy convincente, pero no siempre dice la verdad 💚\n\nEn este momento lo más importante es anclar tu mente al presente. Nombra 5 cosas que puedes ver ahora mismo a tu alrededor. Eso activa el sistema nervioso parasimpático y frena la respuesta de estrés.\n\n¿Cómo de intensa dirías que es tu ansiedad ahora, del 1 al 10?",
    "Que reconozcas lo que sientes ya es el primer paso, y es un paso importante 🌟\n\nLa ansiedad vive en el futuro, en los «¿y si...?» que nos imaginamos. Traer la mente al presente es la mejor herramienta.\n\n¿Hay algo concreto que te preocupa, o es más una sensación general de que algo va a ir mal?",
]

RESPUESTAS_TRISTEZA = [ # Respuestas específicas para cuando se detecta tristeza
    "Gracias por contarme cómo te sientes. La tristeza merece ser escuchada, no ignorada ni apresurada 💙\n\nNo tienes que «ponerte bien» rápido. A veces el cuerpo y la mente necesitan tiempo para procesar.\n\n¿Quieres contarme qué ha pasado? Estoy aquí sin juzgarte.",
    "Lo que sientes es completamente válido 🤍 La tristeza no es debilidad, es una emoción humana que nos dice que algo importa.\n\nCuando estamos tristes el cuerpo necesita movimiento suave, no fuerza. Un paseo corto o un poco de yoga puede liberar neurotransmisores que alivian.\n\n¿Hace cuánto tiempo te sientes así?",
    "Me alegra que estés aquí y que lo hayas compartido 💚 Muchas veces cargar con la tristeza en silencio la hace más pesada.\n\nQuiero preguntarte algo importante: ¿la tristeza tiene una razón concreta o aparece sin saber muy bien por qué?",
]

RESPUESTAS_ESTRES = [ # Respuestas específicas para cuando se detecta estrés
    "El estrés es una señal de que tu cuerpo está trabajando al límite 💙 No es malo sentirlo, pero sí necesita atención.\n\nLo primero es liberar un poco de esa tensión física. Aprieta los puños con fuerza durante 10 segundos y suelta. Repite tres veces. Eso activa el sistema parasimpático.\n\n¿Qué está generando más estrés ahora mismo?",
    "Cuando todo se acumula es difícil saber por dónde empezar 🌱 Pero no tienes que resolver todo hoy.\n\nPropongo algo: escribe tres cosas que PUEDES controlar ahora mismo. Eso ayuda al cerebro a salir del modo pánico.\n\n¿Te parece bien intentarlo?",
    "El estrés crónico agota enormemente 💚 Tu cuerpo lleva tiempo funcionando en modo emergencia y eso tiene un coste.\n\nLa técnica de respiración de caja (4-4-4-4) es la que usan los Navy SEALs en momentos de alta presión. ¿Quieres que te la guíe ahora mismo?",
]

RESPUESTAS_CANSANCIO = [ # Respuestas específicas para cuando se detecta cansancio
    "El cansancio profundo no siempre se soluciona durmiendo más 💙 A veces es el alma la que está cansada, no solo el cuerpo.\n\n¿Puedes decirme más? ¿Es cansancio físico, mental, emocional... o una mezcla de todo?",
    "Hay días en que el cuerpo dice «hasta aquí» y eso hay que escucharlo 🤍\n\nLo más importante ahora es no exigirte más de lo que puedes dar. ¿Cuándo fue la última vez que descansaste de verdad, sin pendientes en la cabeza?",
    "El agotamiento emocional es real y es serio 💚 No es flojera ni excusa.\n\nCuando estamos muy cansados nuestro cerebro no puede gestionar bien las emociones. Todo parece más difícil de lo que es. ¿Qué es lo que más te está drenando energía últimamente?",
]

RESPUESTAS_ALEGRIA = [
    "¡Me alegra mucho escucharlo! 😊 Ese estado positivo es valioso y merece ser celebrado.\n\n¿Qué ha pasado hoy que te hace sentir así? Me encanta saber qué cosas te generan bienestar.",
    "Qué bien que estés bien 🌟 Aprovecha este momento de energía positiva para hacer algo que te nutra.\n\n¿Quieres practicar alguna técnica para «guardar» un poco de este estado y poder volver a él cuando lo necesites?",
    "¡Genial! 💚 Los momentos de bienestar son tan importantes como trabajar los difíciles.\n\nCuando te sientes bien es el mejor momento para construir hábitos. ¿Hay algo que quieras empezar o reforzar hoy?",
]

RESPUESTAS_CRISIS = [
    "Lo que me estás contando es muy serio y quiero que sepas que no estás solo/a 💙\n\nAhora mismo lo más importante es que estés a salvo. Si estás pensando en hacerte daño, por favor llama al 024. Es gratuito, confidencial y están disponibles ahora mismo.\n\n¿Estás en un lugar seguro ahora mismo?",
    "Gracias por contarme esto. Sé que no es fácil decirlo 🤍\n\nQuiero que sepas que lo que sientes tiene solución, aunque ahora no lo parezca. Por favor llama al 024 o al 112 si crees que puedes hacerte daño.\n\n¿Hay alguien de confianza a quien puedas llamar ahora mismo?",
]

RESPUESTAS_GENERALES = [
    "Gracias por compartir eso conmigo 💙 Estoy aquí para escucharte sin juzgarte.\n\n¿Hay algo más que quieras contarme sobre cómo te sientes?",
    "Entiendo 💚 A veces es difícil poner en palabras lo que sentimos, y está bien.\n\n¿Puedes decirme un poco más? Cuanto más me cuentes, mejor puedo acompañarte.",
    "Estoy aquí contigo 🌱 No tienes que pasar por esto solo/a.\n\n¿Cómo llevas el día en general?",
    "Gracias por estar aquí y por compartir cómo te sientes 💙\n\n¿Hay algo concreto en lo que pueda ayudarte ahora mismo?",
]

DETECCION_EMOCIONES = {
    "ansiedad": [
        "ansios", "angustia", "pánico", "nervios", "agobiad", "preocup",
        "miedo", "terror", "fobia", "inquiet", "intranquil", "tenso",
        "no puedo respirar", "corazón acelerado", "temblor", "sudor frío",
        "catastróf", "y si", "peor escenario",
    ],
    "tristeza": [
        "triste", "llorar", "llorando", "deprimid", "depres", "vacío",
        "sin ganas", "no me importa", "para qué", "inútil", "fracas",
        "nada tiene sentido", "no vale la pena", "soy un desastre",
        "todo mal", "nadie me quiere", "estoy solo", "abandonad",
    ],
    "estres": [
        "estresad", "agotad", "presión", "no doy abasto", "demasiado",
        "overwhelm", "saturad", "no puedo más", "al límite", "burned",
        "quemad", "no llego", "todo a la vez", "colapso",
    ],
    "cansancio": [
        "cansad", "agotad", "sin energía", "exhausto", "no tengo fuerzas",
        "sin motivación", "todo me cuesta", "pesadez", "lentitud",
        "no me levanto", "duermo mucho", "duermo poco",
    ],
    "alegría": [
        "bien", "genial", "feliz", "contento", "alegre", "animad",
        "con energía", "positivo", "ilusionad", "emocionad", "orgullos",
        "logré", "conseguí", "éxito", "celebrar",
    ],
    "crisis": [
        "suicidio", "morir", "matarme", "hacerme daño", "no quiero vivir",
        "quitarme la vida", "acabar con todo", "no quiero seguir",
        "mejor sin mí", "desaparecer para siempre",
    ],
}

SUGERENCIAS_TECNICAS = {
    "ansiedad": [
        {"nombre": "🫁 Respiración 4-7-8", "ruta": "/respiracion",       "por_que": "Activa el sistema nervioso parasimpático en minutos"},
        {"nombre": "🌍 Grounding 5-4-3-2-1","ruta": "/tecnica-5-4-3-2-1","por_que": "Ancla tu mente al momento presente"},
        {"nombre": "🧘 Meditación guiada",  "ruta": "/meditacion",        "por_que": "Calma los pensamientos acelerados"},
    ],
    "tristeza": [
        {"nombre": "🏃 Ejercicio terapéutico","ruta": "/ejercicio",       "por_que": "Libera endorfinas y serotonina naturalmente"},
        {"nombre": "📓 Diario emocional",    "ruta": "/diario",            "por_que": "Procesar la tristeza escribiendo alivia enormemente"},
        {"nombre": "💗 Autocompasión",       "ruta": "/autocompasion",     "por_que": "Aprender a tratarte con amabilidad"},
    ],
    "estres": [
        {"nombre": "💆 Relajación muscular", "ruta": "/relajacion-muscular","por_que": "Libera la tensión física acumulada"},
        {"nombre": "🫁 Respiración de caja", "ruta": "/respiracion",        "por_que": "Técnica de alta efectividad para el estrés agudo"},
        {"nombre": "🏃 Ejercicio liberador", "ruta": "/ejercicio",           "por_que": "Quemar el exceso de cortisol"},
    ],
    "cansancio": [
        {"nombre": "🍵 Infusiones relajantes","ruta": "/infusiones",        "por_que": "Ritual de autocuidado y descanso"},
        {"nombre": "🎵 Sonidos relajantes",   "ruta": "/sonidos",            "por_que": "Ambiente sonoro para recuperar energía"},
        {"nombre": "🤸 Yoga suave",           "ruta": "/yoga",               "por_que": "Movimiento suave que restaura la energía"},
    ],
}